...
pointer_array count(int size, int *old_count, int num_p, int numtasks, int r)
{
	...
	for (int i = 0; i < numtasks; i++)
	{
		...
		{
			count[i] = min(min(total_sum-numbers_below, nums_needed), old_count[i]);
...
pointer_array send_receive(int numtasks, int *send, int *receive_count, int send_size, MPI_Comm comm, int rank)
{
	...
	// find the total size of the incoming numbers
	for (int i=0; i< numtasks; i++)
	{
 ...
 *
 * @param numtasks	The total number of processors.
 * @param size		The size of the local numbers.
...
void sort(int numtasks, int size, int *numbers, MPI_Comm comm, int rank, int tot_size, int round)
{
	...
	else
	{
		...
		*****************************************************************/
        ...
        MPI_Allgather(&asize, 1, MPI_INT, nums_above, 1, MPI_INT, comm);
        MPI_Allgather(&bsize, 1, MPI_INT, nums_below, 1, MPI_INT, comm);
		...
		if (tot_above == 0)
		{
			...
			if (rank == proc)
			{
				bsize--;
				asize++;
				parted_nums[size+k] = parted_nums[size-1];
				parted_nums[size-1] = nk;
			}
		}
		...
		*****************************************/
		...
		MPI_Errhandler_set(MPI_COMM_WORLD, MPI_ERRORS_RETURN);
		MPI_Comm_split(comm, color, rank, &next);
...
void slave(int numtasks, int rank)
{
	...
	int *count, *list, *index, *numbers, size, tot_size;
	...
	MPI_Scatterv(list, count, index, MPI_INT, numbers, size, MPI_INT, 0, MPI::COMM_WORLD);
	...
	if (color == 0)
	{
		...
		// send all of the numbers back to the master processor
		int *all_sizes, *master_list, *disp, *numbers;
		int size = 0;
		MPI_Gather(&size, 1, MPI_INT, all_sizes, 1, MPI_INT, 0, MPI_COMM_WORLD);
		MPI_Gatherv(numbers,0,MPI_INT,master_list,all_sizes,disp,MPI_INT,0,MPI_COMM_WORLD);
	}
	*/
	
	// run the function that sorts the numbers
	sort(numtasks,size,numbers,comm,rank,tot_size,0);
}
